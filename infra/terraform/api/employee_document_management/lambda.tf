...
resource "aws_lambda_function" "lambda_function_employee_documents" {
...
resource "aws_lambda_permission" "permissions_employee_documents" {
  for_each = local.methods_names

  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.lambda_function_employee_documents[each.key].function_name
  principal     = "apigateway.amazonaws.com"

  source_arn = "arn:aws:execute-api:${var.aws_region}:${var.account_id}:${var.rest_api.id}/*/${aws_api_gateway_method.methods_employee_documents[each.key].http_method}${local.resources[each.value.resource].path}"
}
