...
class EmployeeDatabaseInsertion:
    ...
    def __init__(self, token, app_name, bucket_folder_name, organization_id, cred_organization_id):
        ...
        self.org_folder = (
            ...
            f'{cred_organization_id}_{app_name}'
        )
        self.employee_id_map = json.load(open(
            f'original_json_objects/id_mappings/'
            f'{cred_organization_id}_{self.app_name}/final/employee_id_map.json')
        )
        self.time = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

        self.references_id_mapping = {}


    def begin_extraction(self):
        with DataBase.get_session() as db:
            self.create_employee_reference(db)
            self.save_mappings()
            db.commit()

    def save_mappings(self, upload_to_s3=True):
        base_folder = (f'original_json_objects/id_mappings/'
                       f'{self.cred_organization_id}_{self.app_name}/final/')
        ...
        # mappings
        #with open(os.path.join(base_folder, 'references_mapping.json'), "w") as f:
        #   f.write(json.dumps(self.employee_id_map))
        with open(os.path.join(base_folder, 'references_id_mapping.json'), "w") as f:
            f.write(json.dumps(self.references_id_mapping))
    ...
    def create_employee_reference(self, db):
        new_base = ''
        for k,v in self.employee_id_map.items():
            employee_profile = db.query(EmployeeProfile).filter_by(employee_id=v).first()
            organization = db.query(Organization).filter_by(id=self.organization_id).first()
            new_base = f'{self.org_folder}/employees/{k}'
            if os.path.exists(f'{new_base}/references.json'):
                references_list = json.load(open(f'{new_base}/references.json'))
                for reference in references_list:
                    new_reference = EmployeeReference()
                    new_reference.reference_id = DataBase.generate_uuid()
                    self.references_id_mapping[reference['id']] = new_reference.reference_id
                    new_reference.employee_id = v
                    if 'file' in reference:
                        new_reference.referee_name = reference['file']['originName']
                        new_reference.created_date = self.parse_text_to_date(reference['file']['created'])
                        new_reference.status = reference.get('status')
                        path_file = f'{new_base}/references/{reference["id"]}'
                        files = os.listdir(path_file)
                        if len(files) > 0:
                            with open(f'{path_file}/{files[0]}', "rb") as file:
                                document_bytes = file.read()
                                path = (f"{STAGE}/app_data/orgs/{organization.name} {DataBase.get_now().year}/"
                                        ...
                                        f"{new_reference.referee_name.split('.')[-1]}")
                                new_reference.s3_path = upload_file_to_s3(path, document_bytes)
                        else:
                            raise Exception('Error document doesnt have a file')
                    else:
                        new_reference.referee_name = reference.get('fullName')
                        new_reference.referee_email = reference.get('email')
                        new_reference.referee_phone = reference.get('phone')
                        new_reference.status = reference.get('status')
                        completion_date = reference.get('resolutionDateTime')
                        new_reference.completion_date = datetime.datetime.strptime(
                            completion_date, '%Y-%m-%dT%H:%M:%S'
                        ) if completion_date else None
                        new_reference.created_date = self.parse_text_to_date(reference.get('firstEmailSendDateTime'))
                        path_file = f'{new_base}/references/{reference["id"]}.json'
                        if os.path.exists(path_file):
                            with open(path_file, "rb") as file:
                                ...
                                path = (f"{STAGE}/app_data/orgs/{organization.name} {DataBase.get_now().year}/"
                                        f"Ongoing/{employee_profile.get_name()} - "
                                        f"{employee_profile.facility}/References/"
                                        f"{employee_profile.get_name()} - Reference {new_reference.reference_id}.json")
                                new_reference.s3_path = upload_file_to_s3(path, document_bytes)
    ...
    def parse_text_to_date(self, created_date):
        if created_date:
            datetime.datetime.strptime(
                created_date, '%Y-%m-%dT%H:%M:%S'
            )
        else:
            return None
